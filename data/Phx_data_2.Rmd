...
phx.dat.2015.3 <- merge(occupancy.pop.2015, income.pop.2015, by.x="geoid", by.y="geoid", all.x=T)
...
phx.dat.2015 <- merge(phx.dat.2015.7, phx.dat.2015.8, by.x="geoid", by.y="geoid", all.x=T, all.y=F)
...
phx.dat <- merge(phx, phx.dat, by.x="id", by.y="GISJOIN", all.x=F, all.y=F)

final.phx <- phx.dat[order(phx.dat$order),]
...
ggplot() +
  geom_polygon(data = phx.dat, 
               aes(x = long, y = lat, group = group, fill = phx.dat$Total_2010), 
