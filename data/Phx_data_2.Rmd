...
phx$geoid <- paste0( "G", phx$STATEFP10, 
...
NHGIS_data_map <- merge(NHGIS_data, phx, by.x="GISJOIN", by.y="geoid", all.x=T)
...
phx.dat.2015.3 <- merge(vacancy.pop.2015, sex.pop.2015, by.x="geoid", by.y="geoid", all.x=T)
...
phx.dat.2015.8 <- merge(phx.dat.2015.5, phx.dat.2015.6, by.x="geoid", by.y="geoid", all.x=T)

phx.dat.2015 <- merge(phx.dat.2015.7, phx.dat.2015.8, by.x="geoid", by.y="geoid", all.x=T)

#Combine datasets for 2010 and 2015

phx.dat <- merge(phx.dat.2010, phx.dat.2015, by.x="geoid", by.y="geoid", all.x=T)

#Combine ACS 5 datasets with NHGIS data

phx.dat$geoid2 <- paste0( "G", phx$STATEFP10, 
                     "0", phx$COUNTYFP10, "0", phx$TRACTCE10 )


phx.dat <- merge(NHGIS_data, phx.dat, by.x="GISJOIN", by.y="geoid2", all.x=T)

#Combine dataset with shapefile

phx.dat <- merge(phx, phx.dat, by.x="geoid", by.y="GISJOIN", all.x=T)


#Convert large data file into GeoJson & write as GeoJson in shapefiles folder

geojson_write( phx.dat, file = "../shapefiles/phx.geojson" )
...
NHGIS_Data_Rename1 <- merge(phx, NHGIS_data, by.x="geoid", by.y="GISJOIN")
NHGIS_mapdata <- merge(phx, NHGIS_data, by.x="geoid", by.y="GISJOIN")
```
...
proj4string( NHGIS_data_map )


##Ethnicity
hispanic_1990 <- ggplot(NHGIS_mapdata) + 
     geom_sf(aes(fill=NHGIS_mapdata$Hispanic.Latino1990), color="white") +
     theme_void() +
...
hispanic_2000 <- ggplot(NHGIS_Data_Rename1) + 
...
hispanic_2010 <- ggplot(NHGIS_Data_Rename1) + 
