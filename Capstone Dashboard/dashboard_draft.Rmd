...
selectInput(inputId= "select", label = "Select Year", 
...
```{r}
renderPlot({
  hist(race.fortified$white_1990, probability = TRUE, breaks = as.numeric(input$select))
  })
...
```{r, chunk-input2, include=TRUE, cache=FALSE}
...
```{r, render-2, include=FALSE}
renderPlot({
  hist(ethnicity.fortified$Hipsanic_Percent_1990, input$select)
  })
...
selectInput(inputId = "radio", label = h3("Select a year"), choices = list("1990" = 1990, "2000" = 2000, "2010" = 2010, "2015" = 2015), selected=2000 )
